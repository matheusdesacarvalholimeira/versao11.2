...
$sqlmat = mysqli_query($conexao, "INSERT INTO materias (materia, professor) VALUES ('$materia', '$professor')");

while ($user_data2 = mysqli_fetch_assoc($resultma)) {
    
    $open = fopen($user_data2["materia"] . '.php', 'a+');
    $escrever = fwrite($open, '
    <?php 
    ...
    while ($prova = mysqli_fetch_assoc($result2)) {
        ...
        echo "</div>";
        echo "<div class="provajus">";
        echo "<p class="multlinha">" . $prova["gabarito"] . "</p>";
        echo "</div>";
...
header('location: cad_materias.php');
